...
| 方法 | 時刻の一致 | 画質 | 備考 |
|---|---|---|---|
| **YouTube Studio からダウンロード** | **○ そのまま使える** | 1080pまで | **推奨。** YouTube公式機能。1日5本まで |
| OBSのローカル録画 | **✗ ずれる** | 無劣化 | `--offset` で補正が必要 |
| リプレイバッファ | ✗（60秒なので不要） | 無劣化 | アップロードが軽い |

顧客は **YouTubeのプレイヤーを見ながら時刻を伝えてくる**。YouTube Studio から
ダウンロードした動画ならその時刻がそのまま使えるが、OBSのローカル録画は
待機画面の分だけずれるため補正が要る。

**yt-dlp 等での自動ダウンロードは YouTube 利用規約違反のため使わない。**
...
```bash
python clip.py peek --in "配信.mp4" --at 1:25:30
```

指定時刻とその前後5秒の静止画を `work/peek/` に出力する。
**顧客の言う時刻が実際にその場面かを、処理を回す前に確かめる。**

ずれていたら補正する。

```bash
python clip.py peek --in "配信.mp4" --at 1:25:30 --offset -90    # 90秒前へ
```
...
| オプション | 内容 |
|---|---|
| `--offset 秒` | 依頼された時刻に加える補正。OBS録画など時間軸がずれた素材で使う |
| `--source-crop x,y,w,h` | テンプレ適用前に元動画を切り取る（比率）。配信画面左端のコメント欄など、Shortsでは読めない領域を落とす。**縦型にしたときゲーム画面を大きく使えるので効果が大きい** |
| `--template A\|B\|C\|D` | レイアウト。既定は `B`。**雑談は `D`（Live2D主役）、ゲーム実況は `B`/`C`（ゲーム画面を含める）** |
...
| `--karaoke` | カラオケ強調を付ける（**既定は単色**）。話者の色分けと配色が衝突するため、複数話者では非推奨 |
...
**既定は単色**。カラオケ強調（`--karaoke`）は次の理由で既定オフにしている。

1. 配色が直感と逆になる。ASSのカラオケは PrimaryColour(発話済み)と
   SecondaryColour(発話前)を切り替えるため、**「これから喋る部分」が着色されて見える**。
2. 話者の色分けと衝突する。話者1の色とカラオケ未発話色がどちらも黄系で、
   コラボ回では「話者が違うのか、まだ喋っていないのか」が区別できない。
...
配信アーカイブ全体を受け取る前提で緩めにしてある。処理は入力側シークで
必要な区間しか読まないため、動画が長くても処理時間はほぼ変わらない。
ボトルネックは転送だけ。**サーバ実装へ移す際は 2GB / 30分 に戻すこと。**

| | |
|---|---|
| ファイルサイズ | 8 GB（`CLIP_MAX_BYTES` で変更可） |
| 動画の長さ | 4時間（`CLIP_MAX_DURATION` で変更可） |
| 解像度 | 3840×2160 まで |
| フレームレート | 60fps まで |
| 切り抜き長 | 5〜180秒 |
...
| 単語タイムスタンプ | **取得できる**（カラオケ強調に必要。既定では未使用） |
