...
# 笑いの途中の息継ぎ。包絡がこのdB以上落ち込む箇所を「間」とみなし、
# 表記に …っ を挟んで息を継いだ感じを出す
LAUGH_BREATH_DROP_DB = 16.0
LAUGH_MAX_BREATHS = 2          # 息継ぎ表記の上限。多いとくどい
# 「ｗ」表記の笑いは斜めに傾けて勢いを出す
LAUGH_SLANT_DEG = -8.0
...
# 発話に合わせて字幕を出す(順次表示)。
# 文章まるごとを先に出すと、言いよどんでいる最中に答えが先に見えてしまい、
# 切り抜きとしてオチが台無しになる。ASSのカラオケ機構を流用し、
# 未発話部分を「透明」にすることで、喋った分だけ現れるようにする。
# 透明でも文字送りの場所は確保されるので、行が揺れない。
REVEAL_ENABLED = True
# 未発話部分の色。アルファFF=完全透明。
# ただしASSでは縁取り(OutlineColour)は別管理なので、未発話でも縁だけ残る。
# 完全に消すには BorderStyle ごと切り替える必要があり、
# 現状は「縁が先に見える」= 文字数の予告になる形で運用する。
# 気になる場合は config.REVEAL_ENABLED = False で従来表示に戻せる。
COLOR_KARAOKE = "&HFF000000"
...
# 効果音は笑い声と同時に鳴るため、音量差があると完全にマスクされる。
# -18dB→-6dB でも実測の差分が0.2dBしかなく聞こえなかったので 0dB にした。
# 生成音側も振幅を上げ、耳につきやすい2-4kHz成分を足してある。
SE_VOLUME_DB = 0.0
