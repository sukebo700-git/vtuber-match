...
export async function POST(request: Request) {
  ...
  const candidateDocs = await findFirestoreLoginApplications(db, email);
  const matchedDocs = candidateDocs.filter((item) => (
    item.data().creator_password_hash === passwordHash &&
    isActiveApplication(item.data())
  ));
  ...
  await recordFirestoreCreatorLogin({
    ...
    console.error("Creator login history could not be recorded:", message);
  });
...
async function findFirestoreLoginApplications(db: FirebaseFirestore.Firestore, email: string) {
